        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
          {filteredRepos.map((repo) => {
            ...
            return (
              <div 
                key={repo.id}
                className="glass-panel p-6 bg-white/5 border border-white/10 hover:border-orange-500/20 transition-all flex flex-col justify-between"
              >
                <div>
                  <div className="flex justify-between items-start gap-4 mb-3">
                    <span className="font-bold text-white text-sm truncate tracking-tight">{repo.repoName}</span>
                    {hasAnalysis ? (
                      <span className="px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] font-bold select-none">
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-[11px] font-semibold flex items-center justify-center gap-2 transition-all cursor-pointer ${
                      hasAnalysis 
                        ...
                        : 'bg-gradient-to-r from-orange-500 to-amber-400 hover:bg-orange-500 text-white border border-orange-500/20'
                    }`}
                  >
                    {isAnalyzing ? (
                      <>
          ...
          <div className="relative w-full max-w-xl sm:max-w-xl h-full bg-[#0E0E0E] border-l border-white/10 p-4 md:p-6 flex flex-col justify-between overflow-y-auto">
            ...
            <div>
              <div className="flex justify-between items-start mb-6">
                <div>
                  <span className="text-[10px] text-amber-400 font-bold uppercase tracking-wider">Repository Details</span>
              ...
              {!selectedRepo.analysisResult ? (
                <div className="py-20 text-center flex flex-col items-center gap-4">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-gradient-to-r from-orange-500 to-amber-400 text-white font-semibold text-xs hover:bg-orange-500 flex items-center gap-2 cursor-pointer"
                  >
            ...
            {selectedRepo.analysisResult && (
              <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-white/10 bg-[#0E0E0E] flex gap-4">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-amber-400 text-white text-xs font-semibold hover:bg-orange-500 disabled:opacity-50 transition-colors shadow-lg shadow-orange-500/15 cursor-pointer"
                >
